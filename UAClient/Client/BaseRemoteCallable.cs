...
namespace UAClient.Client
{
    public abstract class BaseRemoteCallable : IDataChangeHandler
    {
        ...
        // Populate variable maps and create subscriptions only when requested by caller.
                ...
                try
                {
                    ...
                    if (string.Equals(childName, "FinalResultData", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        lock (_final_result_nodes)
                        {
                            ...
                            foreach (var v in _final_result_nodes.Values) v.SetClient(RemoteServerClient);
                        }
                        // initial best-effort read of values
                        await PopulateInitialValuesAsync(session, _final_result_nodes.Values);

                        // FinalResultData is read on demand; do not create subscriptions here by default
                    }
                    else if (string.Equals(childName, "Monitoring", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        // Only subscribe to Monitoring variables when explicitly requested
                        if (_monitoring_nodes.Count > 0 && createSubscriptions && subscriptionManager != null)
                        {
                    ...
                    else if (string.Equals(childName, "ParameterSet", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        lock (_parameter_nodes)
                        {
                            ...
                            foreach (var v in _parameter_nodes.Values) v.SetClient(RemoteServerClient);
                        }

                        await PopulateInitialValuesAsync(session, _parameter_nodes.Values);
                        // Parameters are managed via read/write operations; do not subscribe by default
                    }
                }
                catch { }
            }
        ...
        // Subscribe only core variables (Monitoring). Called during bulk subscription setup to avoid expensive full setup.
        public virtual async Task SubscribeCoreAsync(SubscriptionManager subscriptionManager)
        {
            ...
            try
            {
                foreach (var kv in _monitoring_nodes)
                {
                    try
                    {
                        kv.Value.SetClient(RemoteServerClient);
