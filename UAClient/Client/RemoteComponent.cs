...
namespace UAClient.Client
{
    ...
    public class RemoteComponent : IDataChangeHandler
    {
        ...
        // Expose current state as a property (read from Monitoring["CurrentState"] or cached)
        public string? CurrentState
        {
            get
            {
                if (Monitoring.TryGetValue("CurrentState", out var stateVar))
                {
                    return stateVar.Value?.ToString();
                }
                if (Monitoring.TryGetValue("State", out var altStateVar))
                {
                    return altStateVar.Value?.ToString();
                }
                return null;
            }
        }
        ...
        public void DataChangeNotification(NodeId nodeId, DataValue value, MonitoredItemNotificationEventArgs args)
        {
            try
            {
                ...
                if (_monitoring_nodes.TryGetValue(nodeId, out var mv))
                {
                    ...
                    // notify subscribers about state change (provide string representation)
                    try
                    {
                        var stateStr = mv.Value?.ToString();
                        foreach (var sub in _subscribers)
                        {
                            try { sub.OnStateChange(stateStr); } catch { }
                        }
                    }
                    catch { }
                    return;
                                     ...
                                     if (subRefs != null)
                                     {
                                         foreach (var sr in subRefs)
                                         {
                                             try
                                             {
                                                 ...
                                                 var rm = new RemoteMethod(methodName, methodId, RemoteServerClient);
                                                 MethodSet[methodName] = rm;
                                             }
                                             catch { }
                                         }
                                     }
        ...
        // Subscribe core monitoring variables only and propagate to contained skills/components
        public async Task SubscribeCoreAsync(SubscriptionManager subscriptionManager)
        {
            if (subscriptionManager == null) return;
            // set client references and subscribe monitoring variables
            foreach (var kv in _monitoring_nodes)
            {
                try { kv.Value.SetClient(RemoteServerClient); await kv.Value.SetupSubscriptionAsync(subscriptionManager); } catch { }
            }
            // propagate to skills
            foreach (var rs in SkillSet.Values)
            {
                try { await rs.SubscribeCoreAsync(subscriptionManager); } catch { }
            }
            // propagate to nested components
