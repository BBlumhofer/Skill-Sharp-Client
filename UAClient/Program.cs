    ...
    class Program
    {
        ...
        static async Task<int> Main(string[] args)
        {
            ...
            try
            {
                ...
                // Determine normalized mode (accept with or without leading dashes). Default to "isinstorage" when not specified.
                string modeNorm = "isinstorage";
                ...
                if (modeNorm == "skilltest" || modeNorm == "locktest" || modeNorm == "autoready" || modeNorm == "isinstorage")
                {
                    ...
                    case "isinstorage":
                        {
                            try
                            {
                                ...
                                if (targetModule.Storages.Count == 0)
                                {
                                    Console.WriteLine("No storages discovered on module; nothing to test.");
                                    return 1;
                                }

                                var storage = targetModule.Storages.Values.First();
                                // try to find a readable ProductID or CarrierID in slot variables
                                ...
                                if (string.IsNullOrEmpty(foundProductId))
                                {
                                    Console.WriteLine("No readable ProductID/CarrierID found in storage slots or storage variables; nothing to test.");
                                    return 1;
                                }

                                Console.WriteLine($"Found product id to search for: {foundProductId}");
                                var (found, slotFound) = await storage.IsInStorageAsync(client, productId: foundProductId);
                                Console.WriteLine($"IsInStorage -> found={found}, slot={(slotFound != null ? slotFound.Name : "<null>")}");
                                return found ? 0 : 2;
                            }
                            catch (Exception ex)
                            {
                                UAClient.Common.Log.Warn($"IsInStorage test failed: {ex.Message}");
                                return 1;
                            }
                        }
